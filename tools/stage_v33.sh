...
cat > "$PREVIEW/forty-three-emblems.html" <<'HTMLHEAD'
<!DOCTYPE html>
...
<h1>Forty-three pages, forty-three motion mechanics — Operational Runbooks group complete</h1>
...
add_card() {
  ...
  } >> "$PREVIEW/forty-three-emblems.html"
}
...
# ─── v33 cleanup script (deletes orphan stub directories from user's repo) ───
cat > "$REPO/tools/CLEANUP_v33.sh" <<'CLEANUP_EOF'
#!/usr/bin/env bash
...
# were the pre-v33 placeholder seeds for AI System Architecture, AI
# Ethics, AI Monitoring, RAG, and AI Security. The substantive
# replacements now live under content/ai-native/ as part of v33.
#
...
#   git add -A
#   git commit -m "chore: remove content/ai/* orphan stubs (v33 ai-native group)"
...
if [ $REMOVED -eq 0 ]; then
  ...
  echo "  git add -A"
  echo "  git commit -m 'chore: remove content/ai/* orphan stubs (v33 ai-native group)'"
  echo "  git push origin main"
fi
CLEANUP_EOF
chmod +x "$REPO/tools/CLEANUP_v33.sh"

tar czf "$OUT/ascendion-engineering-v33.tar.gz" \
    --exclude='.git' --exclude='node_modules' --exclude='*.pyc' \
    tools src content infra .github

# Also expose CLEANUP_v33.sh as a standalone output (user can run it before
# extracting the tarball, without having to extract the tarball first).
cp "$REPO/tools/CLEANUP_v33.sh" "$OUT/CLEANUP_v33.sh"

# AI-Native preview copies
cp "$PREVIEW/ai-native-architecture-preview.html" "$OUT/ai-native-architecture-preview-v33.html"
cp "$PREVIEW/ai-native-ethics-preview.html"       "$OUT/ai-native-ethics-preview-v33.html"
cp "$PREVIEW/ai-native-monitoring-preview.html"   "$OUT/ai-native-monitoring-preview-v33.html"
cp "$PREVIEW/ai-native-rag-preview.html"          "$OUT/ai-native-rag-preview-v33.html"
cp "$PREVIEW/ai-native-security-preview.html"     "$OUT/ai-native-security-preview-v33.html"

# Governance preview copies (v33)
cp "$PREVIEW/governance-checklists-preview.html"        "$OUT/governance-checklists-preview-v33.html"
cp "$PREVIEW/governance-review-templates-preview.html"  "$OUT/governance-review-templates-preview-v33.html"
cp "$PREVIEW/governance-roles-preview.html"             "$OUT/governance-roles-preview-v33.html"
cp "$PREVIEW/governance-scorecards-preview.html"        "$OUT/governance-scorecards-preview-v33.html"

# Observability preview copies (v33)
cp "$PREVIEW/observability-incident-response-preview.html"  "$OUT/observability-incident-response-preview-v33.html"
cp "$PREVIEW/observability-logs-preview.html"               "$OUT/observability-logs-preview-v33.html"
cp "$PREVIEW/observability-metrics-preview.html"            "$OUT/observability-metrics-preview-v33.html"
cp "$PREVIEW/observability-sli-slo-preview.html"            "$OUT/observability-sli-slo-preview-v33.html"
cp "$PREVIEW/observability-traces-preview.html"             "$OUT/observability-traces-preview-v33.html"

# Runbooks preview copies (v33)
cp "$PREVIEW/runbooks-incident-preview.html"               "$OUT/runbooks-incident-preview-v33.html"
cp "$PREVIEW/runbooks-migration-preview.html"              "$OUT/runbooks-migration-preview-v33.html"
cp "$PREVIEW/runbooks-rollback-preview.html"               "$OUT/runbooks-rollback-preview-v33.html"
...
cp "$PREVIEW/forty-three-emblems.html"                       "$OUT/forty-three-emblems-v33.html"
cp "$PREVIEW/patterns-data-preview.html"                  "$OUT/data-patterns-preview-v33.html"
cp "$PREVIEW/knowledge-graph-preview.html"                "$OUT/knowledge-graph-preview-v33.html"
cp "$PREVIEW/system-design-edge-ai-preview.html"          "$OUT/edge-ai-preview-v33.html"
cp "$PREVIEW/system-design-event-driven-preview.html"     "$OUT/event-driven-preview-v33.html"
cp "$PREVIEW/system-design-ha-dr-preview.html"            "$OUT/ha-dr-preview-v33.html"
cp "$PREVIEW/system-design-scalable-preview.html"         "$OUT/scalable-preview-v33.html"
cp "$PREVIEW/technology-ui-ux-cx-preview.html"            "$OUT/ui-ux-cx-preview-v33.html"
cp "$PREVIEW/technology-api-backend-preview.html"         "$OUT/api-backend-preview-v33.html"
cp "$PREVIEW/technology-databases-preview.html"           "$OUT/databases-preview-v33.html"
cp "$PREVIEW/technology-cloud-preview.html"               "$OUT/cloud-preview-v33.html"
cp "$PREVIEW/technology-devops-preview.html"              "$OUT/devops-preview-v33.html"
cp "$PREVIEW/technology-practice-circles-preview.html"    "$OUT/practice-circles-preview-v33.html"
cp "$PREVIEW/technology-engagement-models-preview.html"   "$OUT/engagement-models-preview-v33.html"
cp "$PREVIEW/security-application-security-preview.html"           "$OUT/application-security-preview-v33.html"
cp "$PREVIEW/security-authentication-authorization-preview.html"   "$OUT/authentication-authorization-preview-v33.html"
cp "$PREVIEW/security-cloud-security-preview.html"                 "$OUT/cloud-security-preview-v33.html"
cp "$PREVIEW/security-encryption-preview.html"                     "$OUT/encryption-preview-v33.html"
cp "$PREVIEW/security-vulnerability-management-preview.html"       "$OUT/vulnerability-management-preview-v33.html"
...
tar tzf "$OUT/ascendion-engineering-v33.tar.gz" | head -30
